  ...
  return (
    <article className="group overflow-hidden rounded-[1.5rem] border border-black/8 bg-white shadow-[0_18px_50px_rgba(28,34,31,0.06)]">
      ...
      <div className="flex min-h-56 flex-col p-5">
        ...
        <div className="mt-auto flex items-end justify-between gap-4 pt-5"><div className="flex flex-wrap items-baseline gap-2"><span className="text-xl font-black text-[var(--ink)]">{formatNaira(product.priceKobo)}</span>{product.previousPriceKobo ? <span className="text-sm text-[var(--muted)] line-through">{formatNaira(product.previousPriceKobo)}</span> : null}</div><span className="shrink-0 text-sm font-black text-[var(--accent-dark)]">View product</span></div>
      </div>
    </article>
  );
}
