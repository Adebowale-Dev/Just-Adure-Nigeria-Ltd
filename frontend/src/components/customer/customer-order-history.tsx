    ...
    <section className="rounded-[2rem] border border-black/8 bg-white p-6 shadow-[0_18px_50px_rgba(28,34,31,.06)] sm:p-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold tracking-[-.025em] text-[var(--ink)] sm:text-3xl">Your purchases</h2>
      ...
      {!orders.length && !error ? (
        <div className="mt-6 rounded-[1.5rem] border border-dashed border-black/15 bg-[#fbfaf6] p-8 text-center">
          ...
          <h3 className="mt-5 text-xl font-semibold tracking-[-.02em] text-[var(--ink)]">No orders yet.</h3>
              ...
              <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="text-base font-semibold text-[var(--ink)] sm:text-lg">{order.orderNumber}</p>
                    <span className={`rounded-full border px-3 py-1 text-xs font-semibold capitalize ${statusBadgeClass(order.orderStatus)}`}>{readableStatus(order.orderStatus)}</span>
                    <span className={`rounded-full border px-3 py-1 text-xs font-semibold capitalize ${statusBadgeClass(order.paymentStatus)}`}>{readableStatus(order.paymentStatus)}</span>
                  </div>
        ...
        <div className="mt-8 overflow-hidden rounded-[2rem] border border-black/8 bg-white shadow-[0_18px_50px_rgba(28,34,31,.05)]">
          <div className="grid gap-5 bg-white p-6 text-[var(--ink)] sm:p-8 lg:grid-cols-[1fr_auto] lg:items-center">
            <div>
              ...
              <h3 className="mt-2 text-2xl font-semibold capitalize tracking-[-.025em]">{readableStatus(selectedOrder.orderStatus)}</h3>
            ...
            <div className="rounded-3xl bg-[#fff3e8] px-5 py-4 text-left lg:text-right">
              ...
              <p className="mt-1 text-xl font-semibold">{formatNaira(selectedOrder.totalKobo)}</p>
            </div>
          </div>

          <div className="grid gap-6 p-6 sm:p-8 xl:grid-cols-[1fr_22rem]">
            <div>
              <div className="flex items-center gap-3"><ReceiptText className="size-5 text-[var(--accent-dark)]" /><h4 className="text-lg font-semibold tracking-[-.02em] text-[var(--ink)]">Items in this order</h4></div>
              <div className="mt-5 grid gap-3">
                {(selectedOrder.items ?? []).map((item) => <div key={`${item.productId}-${item.sku}`} className="rounded-2xl border border-black/8 bg-[#fbfaf6] p-4"><p className="font-semibold text-[var(--ink)]">{item.name}</p><p className="mt-1 text-sm leading-6 text-[var(--muted)]">SKU: {item.sku} | Condition: {item.condition} | Qty: {item.quantity}</p><p className="mt-2 text-sm font-semibold text-[var(--ink)]">{formatNaira(item.lineSubtotalKobo)}</p></div>)}
              </div>
            ...
            <aside className="rounded-[1.5rem] bg-[#fbfaf6] p-5">
              ...
              <h4 className="mt-4 text-lg font-semibold tracking-[-.02em] text-[var(--ink)]">Order timeline</h4>
              <div className="mt-5 grid gap-4">
                {(selectedOrder.statusHistory ?? []).map((history, index) => <div key={`${history.status}-${history.changedAt}`} className="relative border-l border-black/10 pl-5"><span className="absolute -left-[.42rem] top-1 size-3 rounded-full bg-[var(--accent)]" /><p className="text-xs font-semibold uppercase tracking-[.12em] text-[var(--muted)]">Step {index + 1}</p><p className="mt-1 font-semibold capitalize text-[var(--ink)]">{readableStatus(history.status)}</p>{history.note ? <p className="mt-1 text-sm leading-6 text-[var(--muted)]">{history.note}</p> : null}</div>)}
      ...
      <section className="mx-auto max-w-7xl">
        <div className="mb-8 grid gap-5 rounded-[2rem] border border-black/8 bg-white p-6 shadow-[0_18px_50px_rgba(28,34,31,.06)] lg:grid-cols-[1fr_auto] lg:items-center lg:p-8">
          <div>
            ...
            <h1 className="mt-3 font-serif text-3xl font-semibold leading-tight tracking-[-.035em] text-[var(--ink)] sm:text-4xl">Orders and invoices</h1>
