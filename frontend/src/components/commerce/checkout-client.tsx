...
import { AlertTriangle, ArrowLeft, BadgeCheck, CreditCard, LockKeyhole, MapPin, PackageCheck, Truck, UserRound } from "lucide-react";
...
export function CheckoutClient() {
  ...
  const finalTotalKobo = createdOrder?.totalKobo ?? subtotalKobo + deliveryFeeKobo;
  ...
  return (
    <main className="page-shell">
      <section className="mx-auto max-w-7xl px-4 pb-4 pt-8 sm:px-6 lg:px-8 lg:pt-12">
        <a href="/account/addresses" className="inline-flex items-center gap-2 text-sm font-black text-[var(--muted)] hover:text-[var(--accent-dark)]"><ArrowLeft className="size-4" /> Back to addresses</a>
        <div className="mt-6 flex flex-col gap-4 border-b border-black/10 pb-7 sm:flex-row sm:items-end sm:justify-between">
          <div><p className="section-kicker">Checkout</p><h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">Complete your order</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--muted)]">Enter your delivery details, confirm the total, then continue to Paystack.</p></div>
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-4 pb-16 pt-6 sm:px-6 lg:grid-cols-[minmax(0,1fr)_25rem] lg:px-8">
        <form onSubmit={submitCheckout} className="space-y-6">
          ...
          {!createdOrder ? <>
            ...
            <fieldset className="surface-card p-6 sm:p-8">
              <legend className="sr-only">Order notes</legend>
              <div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-full bg-[#fff3e8] font-semibold text-[var(--accent-dark)]">3</span><div><h2 className="text-lg font-semibold">Final details</h2><p className="mt-1 text-sm text-[var(--muted)]">Add a note if there is anything we should know.</p></div></div>
              <label className="mt-6 grid gap-2 text-sm font-bold">Order note <span className="font-normal text-[var(--muted)]">Optional</span><input name="orderNotes" value={form.orderNotes} onChange={updateField} placeholder="Anything we should know?" className={fieldClass} /></label>
            </fieldset>
        ...
        <aside className="surface-card h-fit p-6 lg:sticky lg:top-40">
          <div className="flex items-center justify-between gap-4"><h2 className="text-lg font-semibold">Order summary</h2><a href="/cart" className="text-sm font-semibold text-[var(--accent-dark)] hover:underline">Edit cart</a></div>
          ...
          <dl className="mt-5 grid gap-3 text-sm"><div className="flex justify-between gap-4"><dt className="text-[var(--muted)]">Subtotal</dt><dd className="font-black">{formatNaira(subtotalKobo)}</dd></div><div className="flex justify-between gap-4"><dt className="text-[var(--muted)]">Delivery</dt><dd className="font-black">{deliveryQuote || createdOrder ? formatNaira(deliveryFeeKobo) : "Calculating..."}</dd></div></dl>
          <div className="mt-5 flex items-baseline justify-between border-t border-black/10 pt-5"><span className="font-semibold">Total</span><strong className="text-xl font-semibold tracking-tight">{formatNaira(finalTotalKobo)}</strong></div>
