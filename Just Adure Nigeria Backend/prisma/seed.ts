...
const catalogue = [
  {
    ...
    description:
      "Fully tested iPhone 13 Pro with Face ID, cameras, speakers, charging, and network functions working correctly. The photographs represent the demonstration unit.",
    ...
    featured: true,
    imageUrl:
      "https://images.unsplash.com/photo-1632661674596-df8be070a5c5?auto=format&fit=crop&w=1200&q=85",
    specs: {
      Storage: "256GB",
  ...
  },
  {
    ...
    description:
      "A dependable UK-used Latitude tested for keyboard, ports, Wi-Fi, webcam, battery, display, and storage health. Windows is freshly installed and activated.",
  ...
  },
  {
    ...
    description:
      "The panel has been checked for dead pixels, lines, discolouration, and backlight issues. Streaming apps and external inputs work correctly.",
  ...
  },
  {
    ...
    description:
      "Console has passed disc-drive, network, HDMI, controller, storage, and sustained gameplay tests. It has been cleaned and reset for a new owner.",
  ...
  },
  {
    ...
    description:
      "The appliance completed an extended cooling test. Thermostat, interior light, seals, shelves, freezer, and compressor operate correctly.",
  ...
  },
  {
    ...
    description:
      "All ports have been load-tested. The charger supports dynamic output sharing and is suitable for compatible phones, tablets, and USB-C laptops.",
...
async function main() {
  ...
  for (const grade of conditionGrades) {
    const record = await prisma.conditionGrade.upsert({
      where: { code: grade.code },
      update: grade,
      create: grade,
    });
    gradeByCode.set(record.code, record.id);
  }

  await prisma.deliveryZone.upsert({
    where: { id: "seed-zone-lagos-mainland" },
    update: {
      feeKobo: 7_500_00,
      minDeliveryDays: 1,
      maxDeliveryDays: 2,
  ...
  await prisma.deliveryZone.upsert({
    where: { id: "seed-zone-lagos-state" },
    update: {
      feeKobo: 12_000_00,
      minDeliveryDays: 1,
      maxDeliveryDays: 3,
  ...
  for (const item of catalogue) {
    const brand = await prisma.brand.upsert({
      ...
      update: { name: item.brand, isActive: true },
    ...
    const productData = {
      ...
      conditionGradeId: gradeByCode.get(item.condition)!,
    ...
    await prisma.product.create({
      data: {
        ...
        specifications: {
          create: Object.entries(item.specs).map(([label, value], index) => ({
  ...
  console.info(`Seeded ${catalogue.length} realistic demonstration products.`);
}
